...
<script type="text/javascript" charset="utf-8"> 
	...
	function choose_course () {
		if(($("#inputyear").val() != '') && ( $("#selectclass").val() != '' ) && ($("#selectroom").val() != '') && ($("#selectpart").val() != '')){
		    var form = document.createElement('form');
		    form.setAttribute('method', 'post');
		    form.setAttribute('action', '<?=base_url('index.php');?>/course/v_reactivity/'+$("#inputyear").val()+'/'+$("#selectclass").val()+'/'+$("#selectroom").val()+'/'+$("#selectpart").val() );
			...
			var form = document.createElement('form');
		    form.setAttribute('method', 'post');
		    form.setAttribute('action', '<?=base_url('index.php');?>/course/manage_reactivity/err1');
		    form.style.display = 'hidden';
		    document.body.appendChild(form)
		    form.submit();
		...
		<div class="content">
			<div class="row-fluid">
				<div class="span12">
					<div class="box">
						<div class="box-head">
							<h3>Manage Required Activity</h3>
						</div>
						<div class="box-content box-nomargin">
							<? echo form_open('index.php/course/v_reactivity', $formattr); ?>
								<div class="wizard">
									<h4>Choose a class to manage required activity</h4>
								</div>
								<? 	if($code == 'err2'){
								...
								<div class="control-group">
									...
									<div class="controls">
										<select name="selectpart" id="selectpart" class="input-small">
								...
								<div class="form-actions">
									...
									<input class="navigation_button btn btn-primary" id="submit" name="submit" value="Submit" type="button" onclick="choose_course();"/>
								</div>			
							<? echo form_close(); ?>
						</div>
					</div>
