...
<script type="text/javascript" charset="utf-8"> 
	...
	function add_reactivity () {
	    var form = document.createElement('form');
	    form.setAttribute('method', 'post');
	    form.setAttribute('action', '<?=base_url('index.php');?>/course/m_reactivity/<?=$year?>/<?=$class?>/<?=$room?>/<?=$part?>');
				...
				<div class="span12">
					<div class="box">
						...
						<div class="box-content box-nomargin">
							<div class="wizard">
								<h4>ปีการศึกษา <?=$year?> ภาคการศึกษาที่ <?=$part?> ระดับชั้น<?=$classname?> <?=$roomname?>  <a class='btn pull-right' onclick="add_reactivity();" name='add_reactivity' id='add_reactivity'>Manage Activity</a></h4>
							</div>
							 <? if($result <> ''){								
							...
							<div class="tab-pane active" id="basic">
								<table id="subject_table" class='table table-striped dataTable table-bordered'> 
									<thead> 
										<tr>
											<th>Activity Name</th><th>Hour</th>
										</tr> 
									</thead> 
									<tbody>
									<?
